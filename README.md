...
1. Подгадывать проценты (100% ширина всего контейнера, 100/3 = 33.33 и меняем ширину (через flex-basis) элемента на чуть более меньшую пока не найдем нужный
2. Рассчитать flex-basis используя отступ. Вы можете взять формулу `calc((100% - 2_отступа)/3)`. Размер отступа можно посмотреть в правилах контейнера.
